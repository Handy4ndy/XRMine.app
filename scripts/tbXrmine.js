async function checkBalances(account) {
    ...
    try {
      ...
      const balance = response.result.lines.find(line => line.currency === currencyToCheck).balance;
          ...
          document.getElementById("xrmineBalance").innerText = "Token Balance:" + balance;
    ...
    try {
        ...
        try {
            const response = await client.request({
                command: "account_lines",
                account: "rfXMq3BMX2dTzJtG4pnhr49u6sHkVQXpWL", // Issuer address
            });
        
            console.log(response.result);
            JSON.stringify(response.result);
        
            const currencyToCheck = "58524D696E650000000000000000000000000000"; // Token currency code
            const balance = response.result.lines.find(line => line.currency === currencyToCheck).balance;
